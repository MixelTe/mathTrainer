...
export function formulaTree(num, depth, int = true) {
    if (depth <= 0)
        return { v: num };
    const sign = Lib.chooseRandom(["+", "-", "*", "/"]);
    let a = 0, b = 0;
    switch (sign) {
        case "+":
            a = randomBig(int);
...
export function treeToString(tree, _top = true) {
    if (!tree.sign)
        return `${tree.v}`;
    let sign = {
        "+": "+",
...
function randomSmall(int) {
    let r = Lib.randomInt(11, 100);
    if (!int)
        r /= Math.pow(10, Lib.randomInt(3));
    return r;
}
function randomBig(int) {
