...
export class Task
{
	...
	static DefCount = 1;
	public count = 1;
	private task = Lib.Div();
	private answer = Lib.Div();

	protected genTask(): { task: HTMLDivElement, answer: HTMLDivElement }
	{
...
export class TaskFormulas extends Task
{
	...
	protected genFormula(): { task: HTMLDivElement, answer: HTMLDivElement | number }
	{
		const task = Lib.Div([], "1 + 1 = ?");
		const answer = 2;
		return { task, answer };
...
export function frac(a: number, b: number, one = true, plus = false)
{
	if (a == 0) return Lib.Span([], (plus ? "+ " : "") + "0");
	const minus = a * b < 0;
	a = Math.abs(a);
	b = Math.abs(b);
	if (round(a / b) == 1) return one ? Lib.Span([],
		(minus ? "-" : "") + (plus ? (minus ? " " : "+ ") : "") + "1"
	) : Lib.Span([], minus ? "-" : "");
